...
public class ICrAData extends JFrame {
	...
	private ICrAData() {
		...
		JSpinner spinDigits = new JSpinner(new SpinnerNumberModel(4,1,100,1));
	...
	private void btnAnalysisListener(ActionEvent evt) {
		...
		try {
			...
			if (vec1.size() > 0 && vec2.size() > 0) {
				...
				showMessage("info", "Ordered pair " + methodNames[comboMethod.getSelectedIndex()]);
				makeCalc(res1, res2, comboMethod.getSelectedIndex(), true);
				makeDisplay(strCrit);
				...
				makeCalc(res, null, comboMethod.getSelectedIndex(), false);
				makeDisplay(strCrit);
	...
	/// Display the result
	private void makeDisplay(String strCrit) {
		...
		try {
			if (result != null) {
				...
				if (strCrit.length() > 0 && strCrit.contains(",")) {
					String[] arrCrit = strCrit.substring(13).split(",");
