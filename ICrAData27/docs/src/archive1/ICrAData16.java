...
public class ICrAData extends JFrame {
	...
	private Color darkgreenClr = new Color(0, 160, 0);
	private Color lightblueClr = new Color(0, 160, 255);
	...
	private JComboBox<String> comboType, comboAggr, comboMethod, comboSize, comboExport;
	...
	private MyComboBox comboOptions;
	...
	private ICrAData() {
		...
		spinPoints = new JSpinner(new SpinnerNumberModel(5,1,100,1));
		...
		cbColor.setFont(monoFont);
		cbColor.setSelected(true);
		cbTicks = new JCheckBox("Ticks");
		cbTicks.setFont(monoFont);
		cbGrid = new JCheckBox("Grid");
		cbGrid.setFont(monoFont);
		cbText = new JCheckBox("Text");
		cbText.setFont(monoFont);
		
		comboOptions = new MyComboBox(new JCheckBox[] {cbColor, cbTicks, cbGrid, cbText});
		comboOptions.setToolTipText("Select the plot options.");
		comboOptions.setPreferredSize(dim100);
		comboOptions.setFont(monoFont);
	...
	private void btnAboutListener(ActionEvent evt) {
		...
		MyTextArea text = new MyTextArea(
				...
				"  Changes in versions from 1.3 to 1.6 have been implemented for" + ls +
	...
	private void btnHelpListener(ActionEvent evt) {
		...
		MyTextArea text = new MyTextArea(
				...
				"When " + MU + " > " + AL + " and " + NU + " < " + BE + ", that is Positive Consonance, color is dark-green." + ls +
	...
	private void comboExportTeX() {
		...
		if (result != null) {
			...
			try {
				...
				bw.write(exportTeX( result.get("PlotPoints"), new boolean[] {
						cbColor.isSelected(), cbTicks.isSelected(),
						cbGrid.isSelected(), cbText.isSelected()} ).replace(ls, "\r\n"));
				bw.close();
	...
	/// Export to TeX
	private String exportTeX(double[][] arrPoints, boolean[] plotOptions) {
		...
		res.append("%%% ICrAData TeX Export " + logDate.format(new Date()) + ls +
			...
			(plotOptions[3] ? "\\usepackage{graphicx}" + ls : "") +
			(plotOptions[0] ? "\\usepackage{xcolor}" + ls : "") +
			"\\begin{document}" + ls +
			...
			"\\begin{center}" + ls +
			(plotOptions[0] ? (chkColor.isSelected() ?
				"\\definecolor{myred}{RGB}{255, 0, 0}" + ls +
				"\\definecolor{mygreen}{RGB}{0, 255, 0}" + ls +
				"\\definecolor{myblue}{RGB}{0, 0, 255}" + ls :
					"\\definecolor{myred}{RGB}{255, 64, 255}" + ls +
					"\\definecolor{mygreen}{RGB}{0, 160, 0}" + ls +
					"\\definecolor{myblue}{RGB}{0, 160, 255}" + ls)
				: "") +
			"\\newcommand{\\myrad}{" + ((double)0.05*(int)spinPoints.getValue()) + "}" + ls +
			(plotOptions[3] ? "\\newcommand{\\myticks}{\\scriptsize}" + ls +
				"\\newcommand{\\mytext}{\\normalsize}" + ls : "") +
			"\\setlength{\\unitlength}{20pt} %10pt=4mm" + ls +
			"\\linethickness{0.5pt}" + ls +
			"\\begin{picture}" +
			(plotOptions[3] ? "(11.5,11.5)(-1.5,-1.5)" + ls : "(10,10)" + ls) +
		...
		for (int i = 0; i < arrPoints.length; i++) {
			if (plotOptions[0]) {
				if (arrPoints[i][0] > (double)spinAlpha.getValue() &&
					arrPoints[i][1] < (double)spinBeta.getValue())
					ccolor = "\\color{mygreen}";
				else if (arrPoints[i][0] < (double)spinBeta.getValue() &&
						arrPoints[i][1] > (double)spinAlpha.getValue())
					ccolor = "\\color{myblue}";
				else
					ccolor = "\\color{myred}";
			}
			
			res.append("\\put(" + arrPoints[i][0]*10 + "," + arrPoints[i][1]*10 + "){" + ccolor + "\\circle*{\\myrad}}" + ls);
		}
	...
	private class MyPanel extends JPanel implements Scrollable, MouseListener, MouseMotionListener {
		...
		protected void paintComponent(Graphics g) {
			...
			if (plotOptions[3]) {
				...
				for (int i = 0; i < marks.length; i++) {
					int widthMark = g2.getFontMetrics(sansFont).stringWidth(String.valueOf(marks[i]));
			...
			for (int i = 0; i < points.length; i++) {
				...
				/// The colors
				if (plotOptions[0]) {
					if (points[i][0] > (double)spinAlpha.getValue() &&
						points[i][1] < (double)spinBeta.getValue())
						g2.setColor((chkColor.isSelected() ? Color.GREEN : darkgreenClr));
					else if (points[i][0] < (double)spinBeta.getValue() &&
							points[i][1] > (double)spinAlpha.getValue())
						g2.setColor((chkColor.isSelected() ? Color.BLUE : lightblueClr));
					else
						g2.setColor((chkColor.isSelected() ? Color.RED: violetClr));
				}
			...
			for (int i = 0; i < points.length; i++) {
				...
				if (markCol > -1 && (points[i][3] == markCol || points[i][2] == markCol)) {
					double x = points[i][0]*widthScale+delta;
					double y = heightScale-points[i][1]*heightScale+delta;
					g2.setColor((chkColor.isSelected() ? darkgreenClr : Color.GREEN));
					g2.fill(new Ellipse2D.Double(x-radius, y-radius, 2*radius, 2*radius));
				}
				...
				if (plotMarkRow > -1 && plotMarkRow > -1 &&
					...
					double x = points[i][0]*widthScale+delta;
					double y = heightScale-points[i][1]*heightScale+delta;
					g2.setColor((chkColor.isSelected() ? violetClr : Color.RED));
					g2.fill(new Ellipse2D.Double(x-radius, y-radius, 2*radius, 2*radius));
				}
			}
	...
	private class MyGlass extends JComponent {
		...
		protected void paintComponent(Graphics g) {
			...
			for (int i = 0; i < glassA.size(); i++) {
				...
				g2.setColor((chkColor.isSelected() ? lightblueClr : Color.BLUE));
			...
			public Component getTableCellRendererComponent(JTable table,
				...
				if (col > 0 && row != col-1) {
					if (arrA[row][col-1] > (double)spinAlpha.getValue() &&
						...
						setForeground((chkColor.isSelected() ? Color.GREEN : darkgreenClr));
					
					else if (arrA[row][col-1] < (double)spinBeta.getValue() &&
							arrB[row][col-1] > (double)spinAlpha.getValue())
						setForeground((chkColor.isSelected() ? Color.BLUE : lightblueClr));
