...
public class ICrAData extends JFrame {
	...
	private Color m_color = new Color(244, 240, 236);
	...
	private ICrAData() {
		...
		panel1d.setBackground(m_color);
		panel1d.setLayout(flowLayout);
		panel1d.add(btnAnalysis);
		
		/// Panel 1
		JPanel panel1 = new JPanel();
		panel1.setBackground(m_color);
		...
		m_scrollA.getViewport().setBackground(m_color);
		...
		m_scrollB.getViewport().setBackground(m_color);
			...
			public void run() {
				try {
					while (true) {
						...
						///double x = (0.05*(double)m_spinPointSize.getValue()); /// exception
	...
	private void btnInfoListener(ActionEvent evt) {
		...
		MyTextArea text = new MyTextArea(
				...
				"Download Java from: https://www.oracle.com/java/technologies/downloads/" + m_ls + m_ls);
	...
	private void btnScreenListener(ActionEvent evt) {
		...
		try {
			...
			BufferedImage img = new BufferedImage((int)(this.getContentPane().getWidth()*tx.getScaleX()),
					(int)(this.getContentPane().getHeight()*tx.getScaleY()), BufferedImage.TYPE_INT_RGB);
			Graphics2D g2 = img.createGraphics();
			g2.scale(tx.getScaleX(), tx.getScaleY());
	...
	private void btnSavePNGListener(ActionEvent evt) {
		...
		if (m_result != null) {
			...
			try {
				...
				BufferedImage img = new BufferedImage((int)(resX.getWidth()*tx.getScaleX()),
						(int)(resX.getHeight()*tx.getScaleY()), BufferedImage.TYPE_INT_RGB);
				Graphics2D g2 = img.createGraphics();
				g2.scale(tx.getScaleX(), tx.getScaleY());
				resX.printAll(g2);
				g2.dispose();
	...
	/// https://stackoverflow.com/questions/48140881/how-to-capture-a-screenshot-in-native-hdpi-resolution-using-awt-robot
	
	/// AffineTransform for Graphics2D HiDPI
	/// https://mail.openjdk.org/pipermail/awt-dev/2017-January/012558.html
	/// https://stackoverflow.com/questions/18878353/controling-the-dpi-of-graphics2d-object
