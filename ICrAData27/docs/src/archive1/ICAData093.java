...
public class ICAData extends JFrame {
	...
	private void btnOpenListener(ActionEvent evt) {
		...
		openFrame.add(panelBtn, BorderLayout.NORTH);
	...
	private void btnViewListener(ActionEvent evt) {
		...
		if (result != null) {
			...
			panelBtn.setBackground(backClr);
			panelBtn.setLayout(new FlowLayout(FlowLayout.LEADING, 5, 5));
			panelBtn.add(comboSize);
			panelBtn.add(comboGrid);
			panelBtn.add(comboColor);
			panelBtn.add(btnExport);
			panelBtn.add(btnClose);
			...
			resFrame.add(panelBtn, BorderLayout.NORTH);
	...
	private void btnExportListener(ActionEvent evt) {
		...
		if (result != null) {
			...
			exFrame.add(panelBtn, BorderLayout.NORTH);
	...
	private void comboMethodListener(ItemEvent evt) {
		if (evt.getStateChange() == ItemEvent.SELECTED) {
			if (result != null)
				makeDisplay(result.get(0), comboMethod.getSelectedIndex());
		}
	}
