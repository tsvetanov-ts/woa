...
public class ICAData extends JFrame {
	...
	private ICAData() {
		...
		btnOpen.setToolTipText("Open a GN file.");
		...
		final JComboBox<String> comboSize = new JComboBox<String>(
				new String[] {"600", "800", "1000", "2000", "3000", "5000"});
		comboSize.setToolTipText("Select the plot size in pixels: 600x600, 800x800, etc.");
		...
		final JComboBox<String> comboGrid = new JComboBox<String>(
		...
		comboGrid.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent evt) {
				MyPanel resPanel = (MyPanel)resScroll.getViewport().getView();
				resPanel.setGrid(comboGrid.getSelectedIndex());
				resPanel.repaint();
				resPanel.revalidate();
			}
		});
		
		final JComboBox<String> comboColor = new JComboBox<String>(
				new String[] {"Red/Gray", "Black/White"});
		...
		comboColor.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent evt) {
				MyPanel resPanel = (MyPanel)resScroll.getViewport().getView();
				resPanel.setColor(comboColor.getSelectedIndex());
				resPanel.repaint();
				resPanel.revalidate();
			}
		});
		...
		textA = new JTextArea("# Use Ctrl-C/Ctrl-V shortcuts to copy/paste data from another program here." + ls +
				...
				"4;3;5;9;1" + ls +
				"4;5;6;7;8" + ls + ls);
		textA.addMouseListener(new RightMouse());
		textA.setDragEnabled(true);
		//textA.setBackground(backClrs[2]);
		...
		textB = new JTextArea();
		//textB.setBackground(backClrs[3]);
		...
		panelCenter.add(scrollA);
		panelCenter.add(scrollB);*/
	...
	private void btnViewDataListener(ActionEvent evt) {
		...
		if (result != null) {
			...
			final JComboBox<String> comboData = new JComboBox<String>(
					new String[] {"Input Data", "Criterial Matrix", "Sign Matrix", "Matrix MU", "Matrix NU",
							"Plot Points", "Vector MU", "Vector NU"});
			...
			comboData.addItemListener(new ItemListener() {
				public void itemStateChanged(ItemEvent evt) {
					...
					viewFrame.setTitle("Matrix for " + methodNames[comboMethod.getSelectedIndex()]);
				}
			});
	...
	private void btnExportListener(ActionEvent evt) {
		...
		try {
			...
			showMessage("error", "Could not export graphic: " + chName.getName());
			//ex.printStackTrace();
		}
	}
	...
	private void btnAnalysis(ActionEvent evt) {
	...
	private void makeDisplay(double[][] arrU, int method) {
		...
		try {
			...
			if (result != null) {
				...
				resPanel.setPreferredSize(new Dimension(600, 600));
