...
public class ICAData extends JFrame {
	...
	private String[] methodNames = new String[] {"MU-biased", "NU-biased", "Intended", "Balanced"};
	...
	private ICAData() {
		...
		btnOpen.setToolTipText("Load data into the program.");
	...
	private void btnOpenListener(ActionEvent evt) {
		...
		btnLoad.setToolTipText("Load this data into the program.");
	...
	private void btnViewListener(ActionEvent evt) {
		...
		if (result != null) {
			...
			btnBar.add(comboGrid);
			btnBar.add(comboColor);
			btnBar.add(btnExport);
			btnBar.add(btnClose);
	...
	private Vector<double[][]> makeCalc(double[][] arrU, int method) {
		...
		try {
			...
			if (method != 3) {
				/// MU-biased 0  NU-biased 1  Intended 2
				arrA = makeA(arrS, method);
				arrB = makeB(arrS, method);
				
			} else {
				/// Make MU-biased
				double[][] arrA1 = makeA(arrS, 0);
				double[][] arrB1 = makeB(arrS, 0);
				/// Make NU-biased
				double[][] arrA2 = makeA(arrS, 1);
				double[][] arrB2 = makeB(arrS, 1);
				/// Balanced 3
				arrA = meanValue(arrA1, arrA2);
				arrB = meanValue(arrB1, arrB2);
	...
	/// Elementwise matrix mean value - (A1{1,1} + A2{1,1})/2
	private double[][] meanValue(double[][] arr, double[][] arr2) {
	...
	private String showArray(double[][] arr, String separator, boolean locale) {
		...
		for (int i = 0; i < arr.length; i++) {
			for (int j = 0; j < arr[i].length; j++) {
				if (locale)
					res.append(arr[i][j] + separator);
				else
					res.append(String.valueOf(arr[i][j]).replace(".", ",") + separator);
