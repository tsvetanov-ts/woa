...
public class ICrAData extends JFrame {
	...
	private Color greenClr = new Color(0, 128, 0);
	...
	private JCheckBox chkRowNames, chkColNames, chkTranspose, chkOrdPair, chkCount, chkTable;
	...
	private ICrAData() {
		...
		chkCount = new JCheckBox("C");
		chkCount.setToolTipText("<html>Count the incomparable values for ordered pair. " +
		...
		chkCount.setBackground(Color.WHITE);
		chkCount.setFont(monoFont);
		
		chkTable = new JCheckBox("S");
		chkTable.setToolTipText("Switch to secondary table view.");
		chkTable.setBackground(Color.WHITE);
		chkTable.setFont(monoFont);
		chkTable.addActionListener(new CheckTableListener());
		...
		comboExport = new JComboBox<String>(new String[] {"Export", "ScreenR", "ScreenP", "PNG", "TeX"});
		...
		try {
			if (result != null) {
				...
				final MyTextArea text = new MyTextArea(
						showArray(hmap.get("MatrixMU"), "MatrixMU", separator[0], locale[0], 4, hdrs, true));
	...
	private void btnHelpListener(ActionEvent evt) {
		...
		MyTextArea text = new MyTextArea(
				...
				">>> Right panel" + ls +
				"Screenshot of the program can be created from Export -> ScreenR/ScreenP." + ls +
				"It may differ from what is visible on new versions of Windows (Windows 10 invisible border)." + ls +
				"Export -> PNG/TeX saves the graphic from the panel in the respective format." + ls + ls +
				"Please use Java 64-bit on Windows 64-bit. Check from Control Panel -> Java." + ls + ls);
	...
	/// Export the screen
	private void comboExportScreen(boolean scr) {
	...
	private class CheckTableListener implements ActionListener {
		public void actionPerformed(ActionEvent evt) {
			refreshTable();
		}
	}
		...
		public void itemStateChanged(ItemEvent evt) {
			if (evt.getStateChange() == ItemEvent.SELECTED) {
				if (comboExport.getSelectedIndex() == 1)
					comboExportScreen(true);
				else if (comboExport.getSelectedIndex() == 2)
					comboExportScreen(false);
				else if (comboExport.getSelectedIndex() == 3)
					comboExportPNG();
				else if (comboExport.getSelectedIndex() == 4)
					comboExportTeX();
		...
		if (vOrdPair && checkOrdPair(data)) {
			...
			try {
				...
				if (vType == 0) {
					...
					result = makeStandard(vres1, vres2, vMethod, vOrdPair);
					...
					result = makeSecondOrder(vres1, vres2, vMethod, vOrdPair);
					...
					result = makeAggregated(vres1, vres2, vAggr, vMethod, vOrdPair);
					headerNames[0] = makeHeader(rowNames, colNames, vres1.get(0).length, 0, vTranspose);
					headerNames[1] = makeHeader(rowNames, colNames, vres1.get(0)[0].length, 1, vTranspose);
		...
		/// Normal - one data set
			...
			try {
				...
				if (vType == 0) {
					...
					result = makeStandard(vres, null, vMethod, vOrdPair);
					...
					result = makeSecondOrder(vres, null, vMethod, vOrdPair);
					...
					result = makeAggregated(vres, null, vAggr, vMethod, vOrdPair);
					headerNames[0] = makeHeader(rowNames, colNames, vres.get(0).length, 0, vTranspose);
					headerNames[1] = makeHeader(rowNames, colNames, vres.get(0)[0].length, 1, vTranspose);
	...
	/// Make Standard ICrA
	private HashMap<String, double[][]> makeStandard(
