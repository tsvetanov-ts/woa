...
public class ICrAData extends JFrame {
			...
			if (evt.getStateChange() == ItemEvent.SELECTED) {
				if (result != null) {
					if (comboTable.getSelectedIndex() == 0) {
						/// Create the tables
						MyTable tableA = new MyTable(result.get(3), critNames, MU);
						MyTable tableB = new MyTable(result.get(4), critNames, NU);
						scrollA.setViewportView(tableA);
						scrollB.setViewportView(tableB);
					} else {
						/// Create the tables
						MyTable tableA = new MyTable(result.get(3), critNames, MU);
						MyTable tableB = new MyTable(result.get(5), critNames, "Distance");
						scrollA.setViewportView(tableA);
						scrollB.setViewportView(tableB);
					}
				}
	...
	private class MyTable extends JTable implements MouseListener {
		...
		private MyTable(double[][] arrData, String[] headers, String name) {
			if (arrData != null) {
				data = arrData;
				rows = headers;
		...
		public void mousePressed(MouseEvent evt) {
			...
			//int row = super.getModel().getR this.getModel().getgetRowAtPoint(evt.getPoint());
	...
	private void makeDisplay(double[][] arrU, int method, String strCrit) {
		...
		try {
			...
			if (result != null) {
				...
				/// Create the tables
				MyTable tableA = new MyTable(arrA, critNames, MU);
				MyTable tableB = new MyTable(arrB, critNames, NU);
				scrollA.setViewportView(tableA);
				scrollB.setViewportView(tableB);
