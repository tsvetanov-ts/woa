...
public class ICrAData extends JFrame {
	...
	private JComboBox<String> comboMethod, comboTable, comboSize, comboGrid, comboColor;
	...
	private ICrAData() {
		...
		comboTable = new JComboBox<String>(new String[] {"Two tables", "One table"});
		comboTable.setToolTipText("Select a view for the tables.");
		comboTable.setPreferredSize(dim150);
		//comboView.setFont(monoFont);
		...
		//panelr2.add(btnAnalysis);
		//panelr2.add(btnViewData);
	...
	private void btnViewDataListener(ActionEvent evt) {
		...
		if (result != null) {
			...
			final JComboBox<String> comboData = new JComboBox<String>(
					new String[] {"Input Data", "Criteria Matrix", "Sign Matrix", "Matrix " + MU, "Matrix " + NU,
							"Matrix Distance", "Plot Points", "Vector " + MU, "Vector " + NU, "Vector Distance"});
			comboData.setToolTipText("Select a matrix for display.");
			comboData.setPreferredSize(dim125);
			comboData.setMaximumRowCount(10);
		...
		public String getToolTipText(MouseEvent evt) {
			...
			for (int i = 0; i < arrRect.length; i++) {
				if (arrRect[i].contains(evt.getX(), evt.getY())) {
					return "<html>Row: " + rowNames[(int)Math.round(points[i][2])] +
							"<br/>Column: " + colNames[(int)Math.round(points[i][3] + 1)] + 
		...
		try {
			...
			for (int i = 0; i < lines.length; i++) {
				...
				if (!line.startsWith("#") && line.length() != 0)
					vec.add(line);
				if (line.startsWith("#criterianames:"))
					criteriadata = line;
			}
			...
			makeDisplay(res, comboMethod.getSelectedIndex(), criteriadata);
	...
	private void makeDisplay(double[][] arrU, int method, String criteriadata) {
		...
		try {
			...
			if (result != null) {
				...
				for (int i = 0; i < String.valueOf(arrA.length).length(); i++)
					pRow += "0";
				for (int i = 0; i < String.valueOf(arrA[0].length).length(); i++)
					pCol += "0";
				...
				/// Table row header - first column
				rowNames = new String[arrA.length];
				for (int i = 0; i < rowNames.length; i++)
					rowNames[i] = "C" + rowFormat.format(Integer.valueOf(i + 1));
				
				/// Table column header - account for row header
				colNames = new String[arrA[0].length + 1];
				for (int i = 1; i < colNames.length; i++)
					colNames[i] = "C" + colFormat.format(i);
				
				/// Manual headers
				if (criteriadata.length() > 0 && criteriadata.contains(",")) {
					String[] arrCrit = criteriadata.substring(15).split(",");
					for (int i = 0; i < arrCrit.length && i < rowNames.length; i++) // careful, check for rowNames length
						if (arrCrit[i].trim().length() > 0)
							rowNames[i] = arrCrit[i].trim();
					for (int i = 0; i < arrCrit.length && i < colNames.length-1; i++) // careful, check until colNames-1
						if (arrCrit[i].trim().length() > 0)
							colNames[i+1] = arrCrit[i].trim();
				}
				...
				/// Create the graphic
				double[][] arrPoints = result.get(6);
				MyPanel resPanel = new MyPanel(arrPoints);
				resPanel.setPreferredSize(new Dimension(valSize, valSize));
				comboSize.setSelectedItem(String.valueOf(valSize));
	...
	private Vector<double[][]> makeCalc(double[][] arrU, int method) {
		...
		try {
			...
			result.add(3, arrA);
			result.add(4, arrB);
			result.add(5, arrDist);
			result.add(6, arrPoints);
			
			result.add(7, vectorA);
			result.add(8, vectorB);
			result.add(9, vectorDist);
	...
	private double[][] makeDistance(double[][] arrA, double[][] arrB) {
		...
		for (int i = 0; i < rows; i++)
			for (int j = 0; j < rows; j++)
				res[i][j] = Math.sqrt(arrA[i][j]*arrA[i][j] + arrB[i][j]*arrB[i][j]);
