			...
			for (int i = 0; i < arrRect.length; i++) {
				if (arrRect[i].contains(evt.getX(), evt.getY())) {
					return "<html>Row: " + rowNames[(int)Math.round(points[i][2])] +
							...
							"<br/>\u03BC: " + numFormat.format(points[i][0]) +
							"<br/>\u03BD: " + numFormat.format(points[i][1]) +
							"</html>";
				}
			}
	...
	private void makeDisplay(double[][] arrU, int method, String criteriadata) {
		...
		try {
			...
			if (result != null) {
				...
				/// Value of 30 is 2 digits, so it is 1 zero for R01, C12 
				/// value of 250 is 3 digits, so it is 2 zeros for R003, C111
