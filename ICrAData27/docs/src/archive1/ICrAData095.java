...
public class ICrAData extends JFrame {
	...
	private ICrAData() {
		...
		final Integer[] intDim = new Integer[] {500, 750, 1000, 2000, 3000, 5000};
		...
		/// Items for panel 3
		final JComboBox<Integer> comboSize = new JComboBox<Integer>(intDim);
			...
			public void itemStateChanged(ItemEvent evt) {
				if (evt.getStateChange() == ItemEvent.SELECTED) {
					if (result != null) {
						...
						resPanel.setPreferredSize(new Dimension(intDim[comboSize.getSelectedIndex()], intDim[comboSize.getSelectedIndex()]));
						//resPanel.setMySize(intDim[comboSize.getSelectedIndex()], resScroll.getBounds().width, resScroll.getBounds().height);
						resPanel.repaint();
						resPanel.revalidate();
					}
				}
		...
		scrollA.getViewport().addChangeListener(new MyScrollA());
		scrollA.setBorder(null);
		
		scrollB = new JScrollPane();
		scrollB.setPreferredSize(scrollSize);
		scrollB.getViewport().addChangeListener(new MyScrollB());
		...
		splitCenterRight.setResizeWeight(0.0);
	...
	private void makeDisplay(double[][] arrU, int method) {
		...
		try {
			...
			if (result != null) {
				...
				//resPanel.setMySize(500, resScroll.getBounds().width, resScroll.getBounds().height);
	...
	private class MyPanel extends JPanel implements Scrollable, MouseListener, MouseMotionListener {
		...
		/*private void setMySize(int val, int val2, int val3) {
			int maxval = 0;
			if (maxval < val) maxval = val;
			if (maxval < val2) maxval = val2;
			if (maxval < val3) maxval = val3;
			this.setPreferredSize(new Dimension(maxval, maxval));
		}*/
	...
	private class MyScrollA implements ChangeListener {
	...
	private class MyScrollB implements ChangeListener {
		public void stateChanged(ChangeEvent evt) {
