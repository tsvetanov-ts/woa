...
public class ICrAData extends JFrame {
	...
	private void btnViewDataListener(ActionEvent evt) {
		...
		if (result != null) {
			...
			text.setText(showArray(result.get(0), separator[0], locale[0], 4, header[0], false));
			...
			final JComboBox<String> comboData = new JComboBox<String>(
					new String[] {"Input1", "Input2", "Criteria Matrix", "Sign Matrix",
							"Matrix " + MU, "Matrix " + NU, "Matrix Distance",
							"Plot Points", "Vector " + MU, "Vector " + NU, "Vector Distance"});
			comboData.setToolTipText("Select a matrix for display.");
			comboData.setPreferredSize(dim125);
			comboData.setMaximumRowCount(11);
	...
	private void makeDisplay(String strObj, String strCrit) {
		...
		try {
			if (result != null) {
				...
				/// Manual headers
				if (strObj.length() > 0 && strObj.contains(",")) {
					String[] arrObj = strObj.substring(13).split(",");
	...
	private void makeCalc(double[][] arrU, double[][] arrU2, int method, boolean isPair) {
		...
		try {
			...
			result.add(9, vectorB);
			result.add(10, vectorDist);
