...
public class ICrAData extends JFrame {
	...
	private ICrAData() {
		...
		JButton btnExport = new JButton("PNG");
		btnExport.setToolTipText("Export the graphic as PNG image.");
		btnExport.setPreferredSize(dim70);
		...
		JButton btnExport2 = new JButton("TeX");
		btnExport2.setToolTipText("Export the graphic as TeX file.");
		btnExport2.setPreferredSize(dim70);
		btnExport2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				btnExport2Listener(evt);
			}
		});
		...
		panel2a.add(comboTable);
		panel2a.add(spinDigits);
		panel2a.add(chkCount);
	...
	private void btnViewDataListener(ActionEvent evt) {
		...
		try {
			if (result != null) {
				...
				text.setText(showArray(hmap.get("MatrixMU"), "MatrixMU", separator[0], locale[0], 4, hdrs, true));
				...
				comboData.addItemListener(new ItemListener() {
					public void itemStateChanged(ItemEvent evt) {
						if (evt.getStateChange() == ItemEvent.SELECTED) {
							text.setText(showArray(hmap.get((String)comboData.getSelectedItem()),
	...
	private void btnAboutListener(ActionEvent evt) {
		...
		text.setText("InterCriteria Analysis proposed for the first time by this article:" + ls +
				...
				"  Project No. DFNI-I-02-5 \"InterCriteria Analysis: A New Approach to Decision Making\"," + ls +
	...
	private void btnExport2Listener(ActionEvent evt) {
			...
			if (evt.getStateChange() == ItemEvent.SELECTED) {
				if (result != null) {
					if (comboTable.getSelectedIndex() == 0) {
						...
						MyTable tableA = new MyTable(result.get("MatrixMU"), headerNames[0], MU, true);
						MyTable tableB = new MyTable(result.get("MatrixNU"), headerNames[0], NU, true);
						scrollA.setViewportView(tableA);
						scrollB.setViewportView(tableB);
					} else {
						/// Create the tables for secondary view
						MyTable tableA = new MyTable(result.get("MatrixMU"), result.get("MatrixNU"), headerNames[0], "(" + MU + "," + NU + ")");
						MyTable tableB = new MyTable(result.get("MatrixDist"), headerNames[0], "Distance", false);
						scrollA.setViewportView(tableA);
						scrollB.setViewportView(tableB);
					}
				}
	...
	private class MyTable extends JTable implements MouseListener {
		...
		private MyTable(double[][] arrData, String[] headers, String name, boolean colors) {
			if (arrData != null) {
				...
				this.getInputMap().put((KeyStroke)actCopy.getValue(Action.ACCELERATOR_KEY), "ACTION_COPY");
				this.getActionMap().put("ACTION_COPY", actCopy);
			}
		}
		...
		public void saveTableData() {
			if (result != null) {
				...
				Clipboard cclip = Toolkit.getDefaultToolkit().getSystemClipboard();
				cclip.setContents(ssel, ssel);
			}
		}
		...
		try {
			...
			for (int i = 0; i < lines.length; i++) {
				...
				if (line.startsWith("#objectnames:") && !vTranspose)
					strInput = line.substring(13);
				if (line.startsWith("#criternames:") && !vTranspose)
					strCalc = line.substring(13);
		...
		try {
			...
			if (vec1.size() > 0 && vec2.size() > 0 && vOrdPair) {
				...
				if (vType == 0) {
					...
					headerNames[0] = makeHeader(strCalc, res1.length, vTranspose, true);
					headerNames[1] = makeHeader(strInput, res1[0].length, vTranspose, false);
					...
					if (vector1 != null && vector2 != null) {
						...
						headerNames[0] = makeHeader(strCalc, vector1.size(), vTranspose, true);
						int vsize = vector1.get(0).length;
						headerNames[1] = makeHeader(strInput, (vsize*vsize-vsize)/2, vTranspose, false);
						headerNames[2] = makeHeader(strCalc, vector1.get(0).length, vTranspose, true);
						headerNames[3] = makeHeader(strInput, vector1.get(0)[0].length, vTranspose, false);
					...
					if (vector1 != null && vector2 != null) {
						...
						headerNames[0] = makeHeader(strCalc, vector1.get(0).length, vTranspose, true);
						headerNames[1] = makeHeader(strInput, vector1.get(0)[0].length, vTranspose, false);
	...
	private String showArray(double[][] arr, String arrid, String separator, Locale locale, int digits,
