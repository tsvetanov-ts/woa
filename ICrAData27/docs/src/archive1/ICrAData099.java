...
public class ICrAData extends JFrame {
	...
	private String exportTeX(double[][] arrPoints) {
		...
		res.append("%%% ICrAData TeX export " + logDate.format(new Date()) + ls +
			...
			"\\begin{center}" + ls +
			(vGrid == 3 ? "\\newcommand{\\myticks}{\\footnotesize}" + ls +
				"\\newcommand{\\mytext}{\\normalsize}" + ls : "") +
		...
		if (vGrid == 1) {
			for (int i = 1; i < 10; i++)
				res.append("\\put(" + i + ",-0.2){\\line(0,1){0.4}}" + ls +
					"\\put(-0.2," + i + "){\\line(1,0){0.4}}" + ls);
		}
		...
		if (vGrid == 2) {
			...
			for (int i = 1; i < 10; i++)
				res.append("\\put(" + i + ",-0.2){\\line(0,1){10.4}}" + ls +
					"\\put(-0.2," + i + "){\\line(1,0){10.4}}" + ls);
		}
		...
		if (vGrid == 3) {
			res.append("\\put(5,-1.2){\\makebox(0,0)[cc]{\\mytext Degree of agreement, $\\mu$}}" + ls +
				...
				"\\put(0,-0.5){\\makebox(0,0)[cc]{\\myticks $0$}}" + ls +
				"\\put(10,-0.5){\\makebox(0,0)[cc]{\\myticks $1$}}" + ls +
				"\\put(-0.43,0){\\makebox(0,0)[cc]{\\myticks $0$}}" + ls +
				"\\put(-0.43,10){\\makebox(0,0)[cc]{\\myticks $1$}}" + ls);
			for (int i = 1; i < 10; i++)
				res.append("\\put(" + i + ",-0.2){\\line(0,1){0.4}}" + ls +
					"\\put(" + i + ",-0.5){\\makebox(0,0)[cc]{\\myticks $0." + i + "$}}" + ls +
					"\\put(-0.2," + i + "){\\line(1,0){0.4}}" + ls +
					"\\put(-0.6," + i + "){\\makebox(0,0)[cc]{\\myticks $0." + i + "$}}" + ls);
		}
	...
	private class MyPanel extends JPanel implements Scrollable, MouseListener, MouseMotionListener {
		...
		protected void paintComponent(Graphics g) {
			...
			Font g2Font = new Font(Font.SANS_SERIF, Font.BOLD, 18);
			g2.setFont(g2Font);
			...
			if (plotGrid == 1) {
				...
				for (int i = 0; i < marks.length; i++) {
					g2.draw(new Line2D.Double(delta-line, delta+heightScale*marks[i], delta+line, delta+heightScale*marks[i]));
					g2.drawString("" + marks2[i], (int)(delta-7*line), (int)(delta+heightScale*marks[i]+1.3*line));
				}
				
				for (int i = 0; i < marks.length; i++) {
					g2.draw(new Line2D.Double(delta+widthScale*marks[i], height-delta-line, delta+widthScale*marks[i], height-delta+line));
					g2.drawString("" + marks[i], (int)(delta+widthScale*marks[i]-2.3*line), (int)(height-delta+4*line));
				}
				
				g2.drawString("0", delta-4*line, delta+heightScale);
				g2.drawString("1", delta-4*line, delta);
				
				g2.drawString("0", delta, delta+heightScale+4*line);
				g2.drawString("1", delta+widthScale, delta+heightScale+4*line);
				
				int widthMU = g2.getFontMetrics(g2Font).stringWidth("Degree of agreement, " + MU);
				int widthNU = g2.getFontMetrics(g2Font).stringWidth("Degree of disagreement, " + NU);
				g2.drawString("Degree of agreement, " + MU, (int)(delta+widthScale*marks[4]-widthMU/2), (int)(height-delta+8*line));
