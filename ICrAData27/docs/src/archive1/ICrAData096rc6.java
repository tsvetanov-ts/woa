...
public class ICrAData extends JFrame {
	...
	private ICrAData() {
		...
		final JSpinner spinDigits = new JSpinner(new SpinnerNumberModel(4,1,100,1));
		spinDigits.setPreferredSize(new Dimension(70, 25));
	...
	private void btnViewDataListener(ActionEvent evt) {
		...
		if (result != null) {
			...
			/// Show input data
			text.setText(showArray(result.get(0), separator[0], locale[0], 4));
			...
			comboData.addItemListener(new ItemListener() {
				public void itemStateChanged(ItemEvent evt) {
					if (evt.getStateChange() == ItemEvent.SELECTED) {
						viewFrame.setTitle("View Data " + methodNames[comboMethod.getSelectedIndex()]);
			...
			if (evt.getStateChange() == ItemEvent.SELECTED) {
				if (result != null) {
					if (comboTable.getSelectedIndex() == 0) {
						...
						MyTable tableA = new MyTable(result.get(3), critNames, MU, true);
						MyTable tableB = new MyTable(result.get(4), critNames, NU, true);
						scrollA.setViewportView(tableA);
						scrollB.setViewportView(tableB);
					} else {
						/// Create the tables for secondary view
						MyTable2 tableA = new MyTable2(result.get(3), result.get(4), critNames, "(" + MU + "," + NU + ")");
						MyTable tableB = new MyTable(result.get(5), critNames, "Distance", false);
						scrollA.setViewportView(tableA);
						scrollB.setViewportView(tableB);
					}
				}
	...
	/// Table for data view as String
	private class MyTable2 extends JTable implements MouseListener {
		
		private String[][] data;
		private String[] rows;
		private String[] cols;
		
		private MyTable2(double[][] arrA, double[][] arrB, String[] headers, String name) {
	...
	private void btnAnalysisListener(ActionEvent evt) {
		...
		try {
			...
			makeDisplay(res, comboMethod.getSelectedIndex(), strCrit);
	...
	private void makeDisplay(double[][] arrU, int method, String strCrit) {
		...
		try {
			...
			if (result != null) {
				...
				/// Manual headers
				if (strCrit.length() > 0 && strCrit.contains(",")) {
					String[] arrCrit = strCrit.substring(15).split(",");
				...
				comboColor.setSelectedIndex(0); /// event fired
	...
	/// Display array
	private String showArray(double[][] arr, String separator, Locale locale, int digits) {
